...
MOCK_BAIDU_SERP = """
<html><body>
  <div class="c-container"><h3><a href="https://www.baidu.com/link?url=AAA123">TCP和UDP核心区别详解</a></h3></div>
  <div class="c-container"><h3><a href="https://www.baidu.com/link?url=BBB456">UDP与TCP区别</a></h3></div>
  <div class="c-container"><h3><a href="https://real-answer.example.com/article">跨域直链结果</a></h3></div>
...
def test_resolve_two_hop(monkeypatch):
    ...
    out = deepfetch.resolve(serp, {"deep_fetch": {"enabled": True, "max_pages_per_query": 3, "fallback_to_serp": True, "timeout_per_page": 15}})
    assert len(out) == 1
    d = out[0]
...
def test_resolve_cache_hit_skips_scrape(monkeypatch):
    class FakeCache:
        ...
        def put(self, key, val):
            ...
            self.store[key] = val
    fc = FakeCache()
    monkeypatch.setattr("signal_search.cache.Cache", lambda cfg=None: fc)

    # 单结果链接 SERP：首跑抓取并写缓存，二跑应全命中、不再抓取
    SINGLE = '<html><body><div class="c-container"><h3><a href="https://www.baidu.com/link?url=AAA123">x</a></h3></div></body></html>'
    calls = {}
    def fake_scrape(url, meta=None):
        calls[url] = True
    ...
    cfg = {"deep_fetch": {"enabled": True, "max_pages_per_query": 3, "fallback_to_serp": True, "timeout_per_page": 15},
...
def test_sogou_selector_extracts_redirectors():
    ...
    fx = os.path.join(here, "sogou_tcp_udp.html")
    if not os.path.exists(fx):
        import pytest
        pytest.skip("fixture missing")
    html = open(fx, encoding="utf-8").read()
    links = extract.parse_serp_links(html, "Sogou", None, max_links=10, serp_url="https://www.sogou.com/web?query=TCP")
    assert len(links) >= 5
    # 归一化后含 sogou /link? 重定向器绝对 URL（修复前为 0）
    assert any("sogou.com/link?url=" in l for l in links)
    # 直链 baike.sogou.com 亦保留
    assert any("baike.sogou.com" in l for l in links)
