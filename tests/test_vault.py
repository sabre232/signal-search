...
def _fake_retriever(query, cfg=None, schema=None, prior_evidence=None, docs=None, **kw):
    ...
    return {
        ...
        "uncertainties": [],
        "sources": [{"url": f"https://e.com/{slug}", "snippet": "TCP 面向连接", "text": "TCP 面向连接详情"}],
    }
...
def test_vault_emit_creates_structure(tmp_path):
    ...
    vd = str(tmp_path)
    out = research_mod("TCP 和 UDP 的区别及原理", cfg=_cfg(), tier="L2",
                                retriever=_fake_retriever, vault_dir=vd)
...
def test_vault_resume_skips_repeat(tmp_path):
    ...
    research_mod("TCP 和 UDP 的区别及原理", cfg=_cfg(), tier="L2",
                          retriever=_count, vault_dir=vd)
    first_n = len(calls)
    assert first_n >= 1
    # 第二次 resume
    out2 = research_mod("TCP 和 UDP 的区别及原理", cfg=_cfg(), tier="L2",
                                 retriever=_count, vault_dir=vd, resume=True)
    # 不应再调 retriever
...
def test_vault_idempotent_report(tmp_path):
    ...
    vd = str(tmp_path)
    research_mod("TCP 原理研究", cfg=_cfg(), tier="L2",
                          retriever=_fake_retriever, vault_dir=vd)
...
def test_outline_persist_storm(tmp_path):
    ...
    vd = str(tmp_path)
    research_mod("某主题", cfg=_cfg(), tier="L3",
                          retriever=_fake_retriever, vault_dir=vd)
...
def test_vault_off_is_pure_memory(tmp_path):
    ...
    out = research_mod("TCP 是什么", cfg=_cfg(), tier="L0",
                                retriever=_fake_retriever, vault_dir=None)
