...
from signal_search import search as searxng_client  # searxng_client 已并入 search（fetch → fetch_searxng）
_SX_JSON = json.dumps({"results": [
...
def test_searxng_fetch_curl(monkeypatch):
    monkeypatch.setattr(scrape, "_fetch_system_curl",
                        lambda *a, **k: (200, _SX_JSON, a[0] if a else ""))
    docs = searxng_client.fetch_searxng("python", cfg={"searxng": {"enabled": True, "url": "http://sx"}},
                                web_fetch=None)
