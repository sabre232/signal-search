...
def _fake_curl(urls_seen):
    def _curl(url, headers=None, cfg=None, timeout=12):
        ...
        if "datacenter-web.eastmoney.com" in url:
            # f10 业绩报表
            body = json.dumps({"success": True, "result": {"data": [
                {"SECURITY_CODE": "600519", "SECURITY_NAME_ABBR": "贵州茅台",
                 "REPORTDATE": "2025-03-31 00:00:00",
                 "TOTAL_OPERATE_INCOME": 51443450583.77, "YSTZ": 10.6673989111,
                 "PARENT_NETPROFIT": 26847474238.76, "SJLTZ": 11.56,
                 "BASIC_EPS": 21.38, "WEIGHTAVG_ROE": 12.34, "PARENT_BIPS": 180.5},
            ]}})
...
def test_finance_report_web_fetch_fallback(monkeypatch):
    ...
    def _curl_block(url, headers=None, cfg=None, timeout=12):
        ...
        return 200, json.dumps({"rc": 0, "data": {"f43": "166600", "f58": "贵州茅台",
                 "f60": "166600", "f116": "1", "f168": "35", "f170": "92"}}), None

    def _wf(url):
        return "<html>茅台 f10 财报页原始内容</html>"

    monkeypatch.setattr("signal_search.scrape._fetch_system_curl", _curl_block)
    docs, warns = finance.fetch("600519 财报", cfg={}, web_fetch=_wf)
    rdoc = next((d for d in docs if d.get("source_type") == "finance_report"), None)
    assert rdoc is not None, f"web_fetch 兜底应产出财报 doc，docs={[d['source_type'] for d in docs]}"
