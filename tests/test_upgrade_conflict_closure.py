...
def _fake_retriever(query, **kw):
    return {"findings": "f", "uncertainties": [], "sources": [{"url": "https://e.com/1", "text": "x"}]}
...
def test_conflict_check_fn():
    ...
    out = research_mod("某研究问题", cfg={}, tier="L2", retriever=_fake_retriever,
                                vault_dir=None, conflict_check_fn=cf)
...
def test_closure_fn_stops_early():
    ...
    out = research_mod("深度研究 X", cfg={}, tier="L3", retriever=ret,
                                vault_dir=None, max_iter=3, closure_check=False, closure_fn=cf)
