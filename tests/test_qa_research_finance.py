...
def _fake_finance_fetch(query, cfg=None, web_fetch=None):
    ...
    return [{
        "url": "https://emweb.securities.eastmoney.com/PC_HSF10/NewFinanceAnalysis/Index?type=web&code=S600519",
        "text": text, "snippet": text[:240], "engine": "eastmoney-f10",
        "source_type": "finance_report", "landing_resolved": True,
        "secid": "1.600519", "code": "600519", "name": "贵州茅台",
        "citation": {"key": "eastmoney-f10-600519", "stock_code": "600519",
                     "name": "贵州茅台", "source": "eastmoney-f10"},
    }], []
...
def test_research_finance_schema_adaptation(monkeypatch):
    ...
    res = research("分析贵州茅台最近5年财报变化", cfg={})
