"""signal_search/research.py - 研究编排层（入口服别→分档L0-L3→按档澄清/拆解/派agent）。
...
class _Deps:
    ...
    def retrieve(self, query: str, schema: List[Dict[str, Any]],
                 evidence: List[Dict[str, Any]]) -> Dict[str, Any]:
        return self.retriever(query, cfg=self.cfg, schema=schema,
                              prior_evidence=list(evidence) or None,
                              web_fetch=self.web_fetch, doi_resolver=self.doi_resolver,
                              github_token=self.github_token)


def _heuristic_tier(query: str) -> str:
    q = (query or "").strip()
    if len(q) < 18 and not any(k in q for k in
                               ("为什么", "对比", "区别", "根因", "如何", "分析", "评估", "方案", "研究", "原因")):
        return "L0"
...
def _closure_satisfied(schema: List[Dict[str, Any]], evidence: List[Dict[str, Any]],
                       findings: str) -> bool:
...
def _open_vault(vault_dir: Optional[str], cfg: Dict[str, Any], query: str, tier: str):
    ...
    if vault_dir is None:
        vault_dir = ((cfg or {}).get("research") or {}).get("vault_dir")
    if not vault_dir:
    ...
    if deps.agent_fn:
        ...
        for dim in schema:
            prompt = _DISPATCH_RESEARCHER_TMPL.format(
                dimension=dim["name"], detail_level=dim["detail_level"],
                topic=query, time_range=_time_range(deps.cfg))
...
def _refine_loop(query: str, state: ResearchState, deps: _Deps,
                 max_iter: int, closure_check: bool) -> None:
...
def _persist_vault(vp: str, query: str, state: ResearchState, resume: bool,
                   cfg: Dict[str, Any], export_citations: Optional[str],
    ...
    _vault.save_state(vp, {"completed": list(done), "findings": state.findings,
                           "iterations": state.iterations})


def _detect_conflicts(query: str, state: ResearchState, conflict_check_fn: Optional[Callable],
...
def research(query: str, cfg: Dict[str, Any] = None, tier: str = "auto",
             clarify_fn: Callable = None, agent_fn: Callable = None, fetch_fn: Callable = None,
             max_iter: int = 3, prior_evidence: List[Dict[str, Any]] = None,
             retriever: Callable = None, web_fetch: Callable = None,
             vault_dir: str = None, resume: bool = False, confirm_outline: bool = False,
             depth: str = None, closure_check: bool = True,
             export_citations: str = None, conflict_check_fn: Callable = None,
             closure_fn: Callable = None, doi_resolver: Callable = None,
    ...
    - vault_dir: Research Vault 落盘目录(A1-A5)。默认 None → 从 cfg.research.vault_dir 读（配置默认
      "./research_vault"，即默认开）；显式传 None 关闭回到纯内存。开启时返回 meta.vault 提醒可关。
    ...
    - export_citations: D5 引用导出 "bibtex"/"md"/"both"；vault 开启时在 vault 内写 citations.bib / references.md。
    - conflict_check_fn: 可选回调(query, evidence)->冲突列表；由调用方注入（即 agent 用的 LLM，见 examples/conflict_llm.py 适配器），做 M51 语义冲突检测，结果写 meta.conflicts + 返回 conflicts。库内不调用任何 LLM；默认 None 不检测，零依赖。
    - closure_fn: 可选回调(query, evidence, findings)->bool；提供则 L3 循环用真语义判停（主问题是否已答），优先于关键词启发式（默认 None 维持 D2 启发式）。
    - doi_resolver: 可选回调(arxiv_id)->doi 字符串；提供则学术源(arXiv)按 id 回填 DOI，BibTeX 升级 @article（D3）。库内不内置任何书目源、不读环境变量；默认 None 不回填、退 @misc。书目源由调用方注入（S2/Crossref/自有库皆可）。
    - github_token: 可选字符串/列表；提供则 GitHub 源(B2)用此 token 打官方 API，调用方注入、优选于 env GITHUB_TOKEN。库内仍读 env 作开箱即用兜底，但调用方想用自己的直接传。默认 None → 回落 env GITHUB_TOKEN → 再无则 web_fetch 兜底。
    """
    cfg = cfg or {}
    deps = _Deps(retriever=retriever or orchestrate.retrieve, cfg=cfg,
                 agent_fn=agent_fn, fetch_fn=fetch_fn, web_fetch=web_fetch,
                 closure_fn=closure_fn, doi_resolver=doi_resolver, github_token=github_token)

    # D4 显式深度档位：depth 给定时覆盖 tier 启发式（用户选深度、库选策略）
    tier = _depth_to_tier(depth) or _classify_tier(query, cfg, tier)
    state = ResearchState(query=query, tier=tier, evidence=list(prior_evidence or []),
                          vault_dir=vault_dir, resume=resume, confirm_outline=confirm_outline)
    ...
    if vp is not None:
        ...
        if confirm_outline:
            return _snapshot(state, meta, needs_confirm=True,
...
def research_interactive(query: str, cfg=None, vault_dir=None, **kw) -> Dict[str, Any]:
    """大纲确认交互式研究（A5 真交互，CLI 版，原 research_cli.py 合并入本模块）。
    ...
    if out.get("needs_confirm"):
        ...
        out = research(query, cfg=cfg, **kw2)
    return out
