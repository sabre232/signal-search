"""signal_search/deepfetch.py - 两跳取源：SERP → 落地页（方案 B / §5.5）。
...
LAST_STATS: Dict[str, int] = {"serp_docs": 0, "resolved": 0, "no_links": 0, "unreachable": 0, "dropped": 0}
...
def _fetch_landing(u: str, engine: str, cfg, cache, timeout: int):
    ...
    try:
        if cache:
            c = cache.get(u)
            if c and c.get("text"):
                return (u, c.get("text"), c.get("title", ""), c.get("snippet", ""), c.get("url") or u, True)
        from .scrape import scrape
        serp_exempt = _is_redirector(u, engine)
        lhtml, meta = scrape(u, {"engine": engine, "cfg": cfg,
                                 "serp_exempt": serp_exempt, "timeout": timeout})
        return (u, lhtml, None, None, (meta or {}).get("final_url") or u, False)
    except Exception:
        return (u, None, None, None, u, False)


def resolve(serp_docs: List[Dict[str, Any]], cfg=None, max_pages: int = None) -> List[Dict[str, Any]]:
    ...
    stats = {"serp_docs": len(serp_docs or []), "resolved": 0,
             "no_links": 0, "unreachable": 0, "dropped": 0}
    ...
    for d in serp_docs:
        ...
        for (u, lhtml, _t, _s, final_url, cached) in results:
            try:
                if not lhtml:
                    continue
                from .extract import extract as _ext, is_serp_host
                ...
                if cache and not cached:
                    cache.put(u, {"url": ld["url"], "text": text,
                                  "title": ld.get("title", ""), "snippet": ld.get("snippet", "")})
                resolved = {
                    "url": ld["url"],
                    "raw_html": None,
                    "title": ld.get("title", ""),
