"""signal_search/academic.py - 学术源（arXiv API 取预印本元数据，D3 结构化引用）。
...
def _arxiv_search(query: str, max_results: int = 10, timeout: int = 15) -> List[Dict[str, Any]]:
    ...
    for entry in root.findall(f"{_ATOM_NS}entry"):
        ...
        authors = [a.findtext(f"{_ATOM_NS}name", "") or ""
...
def search(query: str, cfg: Dict[str, Any] = None,
           ...
           max_results: int = 10,
           doi_resolver: Optional[Callable[[str], str]] = None) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    if not docs:
        if web_fetch:
            try:
                ...
                if html:
                    return [{
                        "url": page, "text": str(html), "engine": "arxiv",
                        "source_type": "academic", "landing_resolved": True,
                        "citation": {"key": f"arxiv-{_urlencode(query)[:14]}",
                                     "source": "arxiv-web", "doi": ""},
                    }], warnings
            except Exception:
                pass
