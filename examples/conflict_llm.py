...
- 适配器 make_llm_conflict(llm_fn)：llm_fn(prompt: str) -> str 由调用方注入（即 agent 用的
  LLM，例如 WorkBuddy 当前模型、DeepSeek、任意 OpenAI 兼容端点——由调用方自己持有 key）。
...
def make_llm_conflict(llm_fn: Optional[Callable[[str], str]] = None):
    ...
    def conflict_check_fn(query: str, evidence: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        if llm_fn is None:
            sys.stderr.write("[conflict_llm] 未注入 llm_fn（agent 的 LLM），走确定性兜底（返回 []）\n")
        ...
        sys_prompt = ("你是事实核查助手。判断下列证据片段之间，是否存在针对【同一事实】的"
...
def _mock_llm(prompt: str) -> str:
    ...
    if "特斯拉" in prompt and "2003" in prompt and "2008" in prompt:
        return ('{"conflicts": [{"claim_a": "特斯拉由艾伯哈德和塔彭宁于 2003 年创立",'
...
def _demo():
    ...
    conflicting = [
        {"url": "https://example.com/a", "text": "特斯拉由马丁·艾伯哈德和马克·塔彭宁于 2003 年创立。"},
    ...
    print("含冲突:", json.dumps(fn("特斯拉成立于哪一年？", conflicting), ensure_ascii=False, indent=2))
